#!/usr/bin/env bash
#SBATCH --job-name=n2_fci_grid
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=32
#SBATCH --time=24:00:00
#SBATCH --array=0-43
#SBATCH --output=n2_fci_grid_%A_%a.out
#SBATCH --error=n2_fci_grid_%A_%a.err
#
# Trillium: N2 bond scan — FCI-referenced select+OO (var/NC) then FCI-referenced K.
# Parallelism = one SLURM array task per (bond × select × cost).
#
# Default grid: N–N = 1.2 … 2.2 step 0.1 (11 points; Quasi_Symmetries N2 grid)
# Selects: greedy (4+3) and iterative (n_sym=7, m_round=1)
# Costs: NC, variance
#   → 11 × 2 × 2 = 44 tasks (array 0-43)
#
# STO-3G N2 (10o/14e) is FCI-tractable. Each task:
#   1) make_pyscf_hamiltonian.py n2 <bond> --basis sto-3g
...
#   4) append row → tables/n2/fci_oo_metrics_grid.csv
#
# Do NOT pass --mem-per-cpu. Max wall time is 24h.
#
# Submit from repo root:
#   sbatch scripts/n2/trillium_n2_fci_oo_metrics_grid.sh
#
# Plot when the array finishes:
#   sbatch --dependency=afterok:<ARRAY_JOBID> \
#     --export=ALL,PLOT_ONLY=1 \
#     scripts/n2/trillium_n2_fci_oo_metrics_grid.sh
#
# Optional env:
#   BASIS, MAXITER, M_ROUND, STATES_PER_SECTOR, DIAG_CSV, PLOT_PNG
#   BONDS_CSV="1.1,1.5,2.0,2.5"   # override grid (matches older LNE figure)
...
DIAG_CSV="${DIAG_CSV:-tables/n2/fci_oo_metrics_grid.csv}"
PLOT_PNG="${PLOT_PNG:-tables/n2/fci_oo_metrics_grid_k_sectors_dim.png}"
